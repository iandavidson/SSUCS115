...
what should be done to image. After user picks an option, text display turns to processing. One more click
closes the window.
'''
...
def main():
    ...
    for i in range(len(buttonlist)):
        if clicked_idx == i:
            chosen = True
            button_trans = draw_button(win, button_coor_list[i], button_string_list[i], chosen)
            # do transformation based off of iteration in for loop
            new_image = transform(img, clicked_idx) #???empty_rgb_list
